...
class LectureMonitorScheduler(private val scope: CoroutineScope) : KoinComponent {
    ...
    fun schedule() {
        ...
        Napier.d("🍎 macOS Scheduler: Starting coroutine-based monitoring...", tag = TAG)
        Napier.d("   ✓ Interval: $REPEAT_INTERVAL", tag = TAG)

        monitorJob = scope.launch {
            Napier.d("╔════════════════════════════════════════════════════════════════════╗", tag = TAG)
            Napier.d("║  🍎 macOS Scheduler: Starting (every $REPEAT_INTERVAL)            ║", tag = TAG)
            Napier.d("╚════════════════════════════════════════════════════════════════════╝", tag = TAG)
     ...
     */
    fun cancel() {
        Napier.d("🛑 macOS Scheduler: Cancelling monitoring coroutine...", tag = TAG)
        monitorJob?.cancel()
        monitorJob = null
